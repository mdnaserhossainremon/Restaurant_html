                ...
                <ul class="navbar-nav ml-auto">
                    ...
                    <li class="nav-item">
                        <a class="nav-link" href="user">
                            <p><?php
            $query1="select name from reservation where email='$email'";
            $nameg = oci_parse($conn,$query1);
            while ($row = oci_fetch_assoc($nameg)) {
                echo $row['name']."<br>";
            }?></p>
                        </a>
                    </li>
...
$sel_query="Select m.name,c.quantity,c.ord_date,c.ord_time,c.email,c.total
    ...
    where c.code=m.code
    and c.email='$email'";
$result = oci_parse($conn,$sel_query);
...
$g1="Select r.res_date,r.res_time,r.no_of_ppl
    from reservation r
    where r.email='$email'";
$res2 = oci_parse($conn,$g1);
while($row = oci_fetch_assoc($res2)) { ?>
            <tr>
                <td align="center"><?php echo $count; ?></td>
